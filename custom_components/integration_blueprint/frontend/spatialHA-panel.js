/**
 * spatialHA Panel - WebSocket architecture
 * Frontend NEVER queries directly. All data goes via backend WebSocket
 * through Home Assistant: hass.callWS({type: "spatialHA/get_version"}) -> backend -> HA
 */
class SpatialHAPanel extends HTMLElement {
  ...
  set hass(hass) {
    this._hass = hass;
    // If user is on About tab and we haven't fetched yet, fetch version via WebSocket
  ...
  async _fetchVersion() {
    ...
    this._render();
    try {
      // Strictly via backend WebSocket -> backend straight to Home Assistant
